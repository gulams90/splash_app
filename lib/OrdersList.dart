...
class OrdersList extends StatefulWidget {
  @override
  _OrdersListState createState() => new _OrdersListState();
}
...
class _OrdersListState extends State<OrdersList> {
  ...
  static final String strTitle="Completed Services";
...
//  }
  ...
  initData() async {
    ...
    String orderDate=CommonUtils.getSimpleDateFormatServer(dt);
...
//    Navigator.of(context).pushNamed("/userTypeScreen");
  ...
  void getList(String orderDate){


    try {
      DatabaseReference myRef = database.reference().child(DatabaseUtils.TBL_ORDERS).child(strOrgId);
      Query qry=myRef.orderByChild("orderedDate").equalTo(orderDate);
      CommonUtils.showProgressBar( pr);
      qry.once().then((DataSnapshot snapshot) {
        ...
        if (snapshot!=null) {
          ...
          if(values!=null) {
            ...
            values.forEach((key, values) {
              if (values != null) {
                OrderData bd = OrderData.fromJson(values);
                if (bd != null&&bd.status==StatusUtils.COMPLETED.toString()) {
                  alData.add(bd);
                  alDataAll.add(bd);
                }
              }
  ...
  Widget build(BuildContext context) {
   ...
   final dateField= DateTimeField(
        ...
        onShowPicker: (context, currentValue) async{
          ...
          if(date!=null) {
            String strDate = CommonUtils.getSimpleDateFormatServer(date);
            if (strDate != null) {
              print("date Change : "+strDate);
              getList(strDate);
            }
          }
